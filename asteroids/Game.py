...
class Game:
    def __init__(self):
        ...
        self.player = Player(self.window, screen_width//2, screen_height//2)
        self.asteroids = []
        self.missiles = []
    ...
    def shoot(self):
        if pygame.time.get_ticks() - self.last_missile > self.missile_time:
            self.missiles.append(Missile(self.window, (self.player.x, self.player.y), self.player.angle))
            self.last_missile = pygame.time.get_ticks()
    ...
    def generate_asteroids(self):
        if pygame.time.get_ticks() - self.last_asteroid > self.asteroid_time:
            self.asteroids.append(Asteroid(self.window, (self.player.x, self.player.y)))
            self.last_asteroid = pygame.time.get_ticks()
