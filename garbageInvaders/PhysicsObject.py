...
class PhysicsObject:
    ...
    def collision(self, other):
        ...
        cross1 = x1 >= (other.x) and x1 <= (other.x + other.width) and y1 >= (
            other.y) and y1 <= (other.y + other.height)
        cross2 = x2 >= (other.x) and x2 <= (other.x + other.width) and y2 >= (
            other.y) and y2 <= (other.y + other.height)
        ...
        if self.fall:
            if self.y < (constants.screen_width - self.height):
                self.velocity = (self.velocity[0],
                                 self.velocity[1] + kGravity * ticks)
                self.ground = False
            if self.y >= (constants.screen_width - self.height):
                self.y = constants.screen_width - self.height
                self.ground = True
                self.velocity = (self.velocity[0], 0
                                 if self.velocity[1] > 0 else self.velocity[1])
        self.lastTime = pygame.time.get_ticks()
        self.x += self.velocity[0] * ticks
        self.y += self.velocity[1] * ticks
        self.offScreen = (self.x > constants.screen_width - self.width
                          or self.x < 0)
        self.offScreen = self.offScreen or (
            self.y > constants.screen_width - self.width or self.y < 0)
